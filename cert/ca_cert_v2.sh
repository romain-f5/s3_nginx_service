...
# create an expired client certificate - requires another cnf file
echo "creating a client certificate and key"
openssl genrsa -out expired_client.key 4096
echo "generating the client certificate csr"
openssl req -new -key expired_client.key -out expired_client.csr -config expired_client.cnf
echo "generating the expired client certificate"
openssl x509 -req -in expired_client.csr -CA ca.crt -CAkey ca.key -out expired_client.crt -CAcreateserial -days 0 -sha256 -extfile client_cert_ext.cnf
